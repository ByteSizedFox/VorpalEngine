...
//    return { VK_EXT_DEBUG_UTILS_EXTENSION_NAME, VK_KHR_SURFACE_EXTENSION_NAME, VK_KHR_WAYLAND_SURFACE_EXTENSION_NAME };
//#else
...
inline SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device) {
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR(device, VK::surface, &presentModeCount, nullptr);

    if (presentModeCount != 0) {
        details.presentModes.resize(presentModeCount);
        vkGetPhysicalDeviceSurfacePresentModesKHR(device, VK::surface, &presentModeCount, details.presentModes.data());
    }
